...
async def action_step(state):
  ...
  try:
    # configure prompt dependent on single-state history
    if (prompt.count("{}")==1):
      message = state['messages'][-1]
      formatted_prompt = prompt.format(message.content)
    # configure prompt dependent on order-dependent two-state history
    elif (prompt.count("{}")==2):
...
# Start the operational flow of the agent graph
async def instigate_runtime_flow(interaction, agentspec, input):
  # define the communication channel for this interaction
  global channel
  channel = interaction.channel
